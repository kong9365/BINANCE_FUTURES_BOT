...
>
> ⏸️ **현재 상태(2026-05-21): 데이터 누적 대기.** 전략을 OI-급증(검증 결과 음의 기대값) →
> Donchian/ATR **돌파**로 교체했고, Supabase 영속 + 시장-인덱스/CMC 글로벌 누적까지 구축 완료.
> **데이터 없이 가치 있는 모든 작업 완료.** 남은 일(돌파 walk-forward 검증, P5b BTC 게이트,
> P3 펀딩 페이드)은 **수개월 데이터 축적 후** 진행. 상세·재개 가이드는 §11. 실거래 GO = **HOLD**.
