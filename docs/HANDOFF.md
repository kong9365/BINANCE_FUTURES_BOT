...
>
> 🟥 **현재 상태(2026-05-22): 검증 종료 — 배포 가능한 알파 없음(증거 기반 HOLD).**
> 신뢰 가능한 검증 하네스(`backtesting/portfolio_backtest.py` 등)를 짓고 2년 데이터로 3개
> 전략군을 엄정 검증한 결과 **모두 강건 엣지 없음**: OI-급증(음수), 돌파(1d는 ZEC 단일종목
> 행운 97%·강건성 없음), 펀딩 페이드(사전확정 기준 불합격 PF 0.74). **소액·유동성 무기한·
> 보수적 설정에서 비용 후 신뢰 가능한 체계적 알파 미발견**(단타 80~97% 손실 base rate 정합).
> **결론: 알파 추구 액티브 매매 안 함 → 미거래/보수 또는 단순 보유.** 진짜 성과 = 검증 인프라.
> 향후 어떤 가설이든 **동일 사전확정 기준 통과 시에만** 배포(통과한 것 없음). 상세 = docs/
> STRATEGY_REALISM_REVIEW.md §7 + Supabase `backtest_runs`. 실거래 GO = **HOLD(증거 기반)**.
